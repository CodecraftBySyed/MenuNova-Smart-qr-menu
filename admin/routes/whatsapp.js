...
router.get('/settings', async (req, res) => {
  try {
    ...
    res.status(500).json({ msg: 'Failed to fetch WhatsApp settings' });
  }
});
...
router.post('/settings', auth, async (req, res) => {
  ...
  try {
    ...
    console.error(err.message);
    res.status(500).json({ msg: 'Failed to update WhatsApp settings' });
  }
});
...
router.delete('/settings', auth, async (req, res) => {
  try {
    ...
    console.error(err.message);
    res.status(500).json({ msg: 'Failed to delete WhatsApp settings' });
  }
});
